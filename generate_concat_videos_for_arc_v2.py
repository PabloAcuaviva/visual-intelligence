        ...
        for partition_folder in (root_arc_path / arc_name).iterdir():
            for problem in tqdm(
                ...
                for img_path in (problem_test / "init_image").iterdir():
                    # This is equivalent to just compying and pasting the image
                    interpolate_x_y(
                ...
                for i, perm in enumerate(itertools.permutations(train_video_paths)):
                    ...
                    num_frames_test_context = 5
                    for test_key, test_path in test_inits.items():
                        img = imageio.imread(test_path)
                        for _ in range(
                            num_frames_test_context
                        ):  # We concatenate 5 times instead
                        ...
                        frames = frames[:-num_frames_test_context]

                    if i >= (N_MAX_CONCATENATIONS - 1):
                        break
